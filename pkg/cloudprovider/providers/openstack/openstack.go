...
func (os *OpenStack) CreateVolume(name string, size int, opt *map[string]string, tags *map[string]string) (volumeName string, err error) {
	...
	if opt != nil {
		opts.VolumeType = (*opt)["VolumeType"]
		opts.Availability = (*opt)["Availability"]
	}
	if tags != nil {
		opts.Metadata = *tags
	}
