...
func (az *Cloud) DetachDisk(lun int32, vmName string) error {
	vm, exists, err := az.getVirtualMachine(vmName)
	if err != nil {
		return err
	} else if !exists {
...
func (az *Cloud) GetDiskLun(diskName, diskUri, vmName string) (int32, error) {
	...
	if err != nil {
		...
		return 0, cloudprovider.InstanceNotFound
	}
	disks := *vm.Properties.StorageProfile.DataDisks
	for _, disk := range disks {
		if disk.Lun != nil {
	...
	return 0, fmt.Errorf("cannot find vhd with name %q and uri %q", diskName, diskUri)
}
