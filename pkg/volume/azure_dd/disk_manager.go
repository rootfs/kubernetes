...
type diskManager interface {
	MakeGlobalPDName(disk azureDisk) string
	// Attaches the disk to the kubelet's host machine.
	AttachDisk(b azureDiskMounter) error
	// Detaches the disk from the kubelet's host machine.
	DetachDisk(disk azureDiskUnmounter, mntPath string) error
}

// utility to mount a disk based filesystem
func diskSetUp(manager diskManager, b azureDiskMounter, volPath string, mounter mount.Interface, fsGroup *int64) error {
	globalPDPath := manager.MakeGlobalPDName(*b.azureDisk)
...
func diskTearDown(manager diskManager, c azureDiskUnmounter, volPath string, mounter mount.Interface) error {
	noMnt, err := mounter.IsLikelyNotMountPoint(volPath)
	if err != nil {
		glog.Errorf("cannot validate mountpoint %s", volPath)
		return err
